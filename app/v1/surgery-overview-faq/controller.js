...
const { SurgeryOverviewFAQ, SurgeryMaster } = require("../../../models/index");
const { ObjectId } = require("mongoose").Types;

const addFAQ = async (req, res) => {
  try {
    const content = req.body;
    const data = await common.create(SurgeryOverviewFAQ.model, content);
...
const allFAQ = async (req, res) => {
  try {
    ...
    const data = await common.findAll(SurgeryOverviewFAQ.model, condition);
...
const updateFAQ = async (req, res) => {
  try {
    const { id } = req.params;
    const content = req.body;
    const data = await common.updateById(SurgeryOverviewFAQ.model, id, content);
    return response.success(
...
const deleteFAQ = async (req, res) => {
  try {
    const { id } = req.params;
    await common.updateById(SurgeryOverviewFAQ.model, id, { isDeleted: true }); // Deleting the FAQ data
    return response.success(
...
const findFAQ = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await common.getById(SurgeryOverviewFAQ.model, id);
    return response.success(
...
//add all Overview faq by gurmeet